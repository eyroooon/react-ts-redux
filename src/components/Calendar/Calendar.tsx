...
const groupsEventsByDay = (events: UserEvent[]) => {
...
const Calendar: React.FC<Props> = ({ events, loadUserEvents }) => {
  ...
  let groupedEvents: ReturnType<typeof groupsEventsByDay> | undefined;
  let sortedGroupKeys: string[] | undefined;

  if (events.length) {
    groupedEvents = groupsEventsByDay(events);
  ...
  return groupedEvents && sortedGroupKeys ? (
    <div className="calendar">
      {sortedGroupKeys.map((dayKey) => {
        ...
        const groupedDate = new Date(dayKey);
        const day = groupedDate.getDate();
        const months = groupedDate.toLocaleDateString(undefined, {
          month: 'long',
        });
        return (
          <div key={dayKey} className="calendar-day">
            ...
            <div className="calendar-events">
              {events.map((event) => {
                return <EventItem key={`evebt_${event.id}`} event={event} />;
              })}
            </div>
          </div>
        );
